...
def main():
    ...
    print("开始提取翻译...")
    try:
        # 下面三个函数需要以 export_dir 作为根目录
        extract_translate(export_dir)
        extract_key(export_dir)
        cleanup_backstories(export_dir)

        # 自动导出所有可翻译字段到 extracted_translations.csv（含 DefInjected 和 Keyed）
        csv_path = os.path.join(export_dir, "extracted_translations.csv")
        all_translations = preview_translatable_fields(export_dir, preview=False)
        import csv
        # Keyed 导出
