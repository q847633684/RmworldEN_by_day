...
def migrate_configurations():
    ...
    unified_config = {
        ...
        "user": {
            ...
            "import_prefs": {"merge_existing": True, "backup_before_import": True},
            "api": {
    ...
    if old_preferences_file.exists():
        try:
            ...
            # 备份旧偏好
            shutil.copy2(
                old_preferences_file, backup_dir / "user_preferences.json.backup"
            )
            ...
            if "extraction" in old_prefs:
                ...
                for old_key, new_key in extraction_mappings.items():
                    if old_key in extraction:
                        unified_config["user"]["extraction"][new_key] = extraction[
                            old_key
                        ]
                        migrated_data.append(
                            f"提取偏好: {old_key} -> user.extraction.{new_key}"
                        )
            ...
            if "import_prefs" in old_prefs:
                ...
                for old_key, new_key in import_mappings.items():
                    if old_key in import_prefs:
                        unified_config["user"]["import_prefs"][new_key] = import_prefs[
                            old_key
                        ]
                        migrated_data.append(
                            f"导入偏好: {old_key} -> user.import_prefs.{new_key}"
                        )
            ...
            if "general" in old_prefs:
                ...
                for old_key, new_key in general_mappings.items():
                    if old_key in general:
                        ...
                        migrated_data.append(
                            f"通用设置: {old_key} -> user.general.{new_key}"
                        )
    ...
    if old_paths_file.exists():
        try:
            ...
            migrated_data.append(
                f"记住的路径: {len(old_paths)} 个路径 -> user.remembered_paths"
            )
    ...
    if old_history_file.exists():
        try:
            ...
            migrated_data.append(
                f"路径历史: {len(old_history)} 个类型 -> user.path_history"
            )
