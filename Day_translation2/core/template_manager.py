...
from ..models.exceptions import TranslationError, ConfigError, ImportError as TranslationImportError, ExportError
...
class TemplateManager:
    ...
    def extract_and_generate_templates(self, 
                                     ...
                                     merge_mode: str = "smart-merge") -> List[Tuple[str, str, str, str]]:
        """
        提取翻译数据并生成模板，同时导出CSV
        ...
        try:
            ...
            if output_dir:
                self._generate_templates_to_output_dir(translations, output_dir, en_keyed_dir, structure_choice, merge_mode)
            else:        
                self._generate_all_templates(translations, en_keyed_dir, structure_choice, merge_mode)
    ...
    def _extract_all_translations(self, definjected_mode: str = "defs") -> List[Tuple[str, str, str, str]]:
        ...
        # 提取Keyed翻译
        try:
            keyed_translations = extract_keyed_translations(str(self.mod_dir), self.config.core.source_language)
        ...
        try:
            if definjected_mode == "definjected":
                ...
                src_lang_path = get_language_folder_path(self.config.core.source_language, str(self.mod_dir))
                src_definjected_dir = Path(src_lang_path) / self.config.core.definjected_dir
                
                if src_definjected_dir.exists():
                    definjected_translations = extract_definjected_translations(str(self.mod_dir), self.config.core.source_language)
            ...
            if definjected_mode == "defs":
                defs_translations = scan_defs_sync(str(self.mod_dir), language=self.config.core.source_language)
    ...
    def _generate_all_templates(self, translations: List[Tuple[str, str, str, str]], en_keyed_dir: str = None, structure_choice: str = "original", merge_mode: str = "smart-merge"):
        """生成所有翻译模板"""
    ...
    def _generate_templates_to_output_dir(self, translations: List[Tuple[str, str, str, str]], output_dir: str, en_keyed_dir: str = None, structure_choice: str = "original", merge_mode: str = "smart-merge"):
        """在指定输出目录生成翻译模板结构"""
    ...
    def _save_translations_to_csv(self, translations: List[Tuple[str, str, str, str]], csv_path: str):
        ...
        try:
            ...
            raise ExportError(f"保存CSV文件失败: {str(e)}", output_path=csv_path, export_format="csv")
    ...
    def _handle_definjected_structure_choice(self, def_translations: List[Tuple[str, str, str, str]], export_dir: str, is_internal_mode: bool = False, structure_choice: str = "original", merge_mode: str = "smart-merge"):
        """处理 DefInjected 结构选择逻辑"""
        if not def_translations:
            return
            
        from ..utils.file_utils import get_language_folder_path
        from .exporters import export_definjected_with_original_structure, export_definjected_with_defs_structure
