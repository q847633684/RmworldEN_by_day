...
try:
    # 尝试相对导入 (包内使用)
    from ..models.exceptions import ImportError as TranslationImportError, ProcessingError
except ImportError:
    # 回退到绝对导入 (直接运行时)
    from models.exceptions import ImportError as TranslationImportError, ProcessingError
...
class XMLProcessor:
    ...
    def update_translations(self, tree: ET.ElementTree, translations: Dict[str, str], 
                           generate_key_func=None, merge: bool = True) -> bool:
        """
        更新XML文件中的翻译内容
