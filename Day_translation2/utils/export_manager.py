...
try:
    from ..models.exceptions import ProcessingError, ValidationError, ExportError
...
class ExportManager:
    ...
    def export_translations(self, 
                          ...
                          preserve_manual_edits: bool = True) -> ExportResult:
        """
        导出翻译数据到文件
        ...
        try:
            ...
            if mode == ExportMode.SMART_MERGE:
                ...
                result = self._export_with_merge(
                    translations, output_file_path
                )
            elif mode == ExportMode.BACKUP:
                result = self._export_with_backup(
                    translations, output_file_path
                )
            elif mode == ExportMode.REPLACE:
                result = self._export_with_replace(
                    translations, output_file_path
                )
    ...
    def _export_with_smart_merge(self, 
                               translations: Dict[str, str], 
                               output_file_path: str,
                               preserve_manual_edits: bool = True) -> ExportResult:
        """使用智能合并模式导出"""
    ...
    def _export_with_merge(self, 
                         translations: Dict[str, str], 
                         output_file_path: str) -> ExportResult:
        """使用传统合并模式导出"""
    ...
    def batch_export_translations(self, 
                                translations_by_file: Dict[str, Dict[str, str]],
                                mode: ExportMode = ExportMode.SMART_MERGE,
                                preserve_manual_edits: bool = True) -> List[ExportResult]:
        """
        批量导出翻译文件
...
# 兼容性函数，支持旧代码调用
def export_with_smart_merge(output_file_path: str, 
                          translations: Dict[str, str], 
