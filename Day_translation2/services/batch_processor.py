...
from ..models.exceptions import ProcessingError, ConfigError, ImportError as TranslationImportError
...
class BatchProcessor:
    ...
    def process_multiple_mods(self, 
                            mod_list: List[str], 
                            csv_path: Optional[str] = None, 
                            language: Optional[str] = None) -> OperationResult:
        """
        批量处理多个模组
    ...
    def _process_mods_concurrently(self, mod_list: List[str], csv_path: Optional[str], language: str) -> None:
        """并发处理模组"""
    ...
    def _process_single_mod(self, mod_dir: str, csv_path: Optional[str], language: str) -> ModProcessResult:
        """
        处理单个模组
        ...
        try:
            ...
            if csv_path:
                try:
                    ...
                    logging.debug(f"XML更新完成: {mod_dir}, 处理{files_processed}个文件，更新{files_updated}个")
                except Exception as e:
    ...
    def _generate_processing_result(self) -> OperationResult:
        ...
        if failed_results:
            result.errors = [f"{Path(mod).name}: {res.error}" for mod, res in failed_results.items()]
