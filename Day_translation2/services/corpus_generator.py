...
from ..models.exceptions import ProcessingError, ValidationError, ImportError as TranslationImportError
from ..models.result_models import OperationResult, OperationStatus, OperationType
from ..core.extractors import extract_keyed_translations, extract_definjected_translations
from ..config import get_config


def generate_parallel_corpus(mod_dir: str, output_csv: str, 
                           source_language: str = "English",
                           target_language: str = "ChineseSimplified") -> OperationResult:
    """
    生成英中对照平行语料
...
def _match_translations(source_dict: Dict[str, str], 
                       target_dict: Dict[str, str]) -> List[Tuple[str, str, str]]:
    """匹配源语言和目标语言的对应翻译"""
    try:
        parallel_pairs = []
...
def _save_parallel_corpus(parallel_pairs: List[Tuple[str, str, str]], 
                         output_path: str, statistics: Dict[str, any]) -> None:
    """保存平行语料到CSV文件"""
    try:
        # 确保输出目录存在
        ...
        with open(corpus_csv, "r", encoding="utf-8") as f:
            ...
            for row in reader:
                ...
                if key and source_text and target_text:
                    ...
                    # 计算长度比例
                    length_ratio = len(target_text) / len(source_text) if len(source_text) > 0 else 0
