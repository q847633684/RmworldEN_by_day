...
from ...models.exceptions import ValidationError, ProcessingError
...
class TestTranslationFacade:
    ...
    def test_extract_templates_and_generate_csv_success(self, mock_template_manager, facade, temp_dir):
        """测试模板提取和CSV生成成功"""
        # 模拟模板管理器
        mock_manager = Mock()
    ...
    @patch('Day_translation2.core.template_manager.TemplateManager')
    def test_import_translations_to_templates_success(self, mock_template_manager, facade, temp_dir):
        """测试翻译导入成功"""
        # 创建测试CSV文件
        ...
        result = facade.import_translations_to_templates(
            ...
            templates_directory=str(temp_dir / "templates")
        )
    ...
    def test_generate_corpus_success(self, mock_generate, facade, temp_dir):
        ...
        result = facade.generate_corpus(
            mod_directory=str(temp_dir),
            output_csv=str(temp_dir / "corpus.csv")
        )
