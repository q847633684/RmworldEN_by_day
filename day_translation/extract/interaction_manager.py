...
class InteractionManager:
    ...
    def _confirm_configuration(self, config: Dict[str, Any]) -> bool:
        ...
        print(f"   🗂️ 文件结构：{self._format_structure_description(config['template_structure'])}")
        
        while True:
            choice = input(f"\n{Fore.CYAN}确认以上配置？(y/n/r): {Style.RESET_ALL}").strip().lower()
            if choice in ['y', 'yes', '']:
                return True
            elif choice in ['n', 'no']:
                return False
                ...
                print(f"   {Fore.RED}❌ 请输入 y(确认)/n(取消)/r(重新配置){Style.RESET_ALL}")
    ...
    def _detect_english_directories(self, mod_dir: str) -> Dict[str, Union[bool, Optional[str]]]:
        ...
        has_english_definjected = en_definjected_dir.exists() and any(en_definjected_dir.rglob('*.xml'))
        
        # 检测英文 Keyed 目录
        en_keyed_dir = Path(mod_dir) / "Languages" / "English" / "Keyed"
        has_english_keyed = en_keyed_dir.exists() and any(en_keyed_dir.rglob('*.xml'))
        ...
        if has_definjected:
            ...
            while True:
                ...
                if choice == '1':
                    ...
                    print(f"   {Fore.BLUE}✅ 采用智能推荐：{recommendation['recommended']}{Style.RESET_ALL}")
    ...
    def _handle_output_conflicts(self, output_status: Dict[str, Union[bool, Optional[str]]]) -> str:
        ...
        if has_output_files:
            ...
            if analysis['recommended']:
                print(f"{Fore.GREEN}🤖 智能推荐：{analysis['recommended']} (理由: {analysis['reason']}){Style.RESET_ALL}")
            ...
            while True:
                ...
                choice = input(f"\n{Fore.CYAN}请选择 (1-{max_choice}): {Style.RESET_ALL}").strip()
                if choice == '1':
        ...
        # 3. 如果选择defs_only且非merge，询问用户选择4.2或4.3
            ...
            while True:
                ...
                if choice == '1':
                    ...
                    print(f"   {Fore.CYAN}✅ 选择：按原始Defs文件结构组织{Style.RESET_ALL}")
