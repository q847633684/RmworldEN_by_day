...
def handle_java_translate():
    ...
    try:
        ...
        try:
            ...
            show_error(str(e))
            # 自动构建逻辑
            user_input = input(f"{Fore.YELLOW}是否自动构建Java工具？[y/n]: {Style.RESET_ALL}").strip().lower()
            if user_input == 'y':
                # 构建脚本现在在当前目录下的RimWorldBatchTranslate子目录
                current_dir = os.path.dirname(os.path.abspath(__file__))
                build_script = os.path.join(current_dir, 'RimWorldBatchTranslate', 'build.bat')
                if os.path.exists(build_script):
                    ...
                    show_error("未找到build.bat脚本，请手动进入RimWorldBatchTranslate目录构建。")
