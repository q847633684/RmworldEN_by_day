...
class UnifiedInteractionManager:
    ...
    def show_welcome(self):
        """显示程序欢迎界面"""
        has_api_key = bool(
            self.config.user.api.aliyun_access_key_id or 
        ...
        # 显示当前配置状态
        auto_status = f"{Fore.GREEN}开启{Style.RESET_ALL}" if self.config.user.general.auto_mode else f"{Fore.YELLOW}关闭{Style.RESET_ALL}"
    ...
    def _configure_extraction_preferences(self, mod_dir: str) -> Tuple[bool, Dict[str, Any]]:
        ...
        if os.path.exists(auto_en_keyed_dir):
            ...
            if input(f"{Fore.CYAN}是否使用检测到的目录？[Y/n]: {Style.RESET_ALL}").lower() not in ['n', 'no']:
                ...
                en_keyed_dir = self._get_directory_path("英文Keyed目录", 
                    self.config.get_remembered_path("en_keyed_dir"), required=False)
    ...
    def _get_smart_output_directory(self, name: str, default: str = None) -> Optional[Tuple[str, str]]:
        ...
        if not os.path.exists(output_dir):
            # 目录不存在，询问是否创建
            if input(f"{Fore.CYAN}目录不存在，是否创建？[Y/n]: {Style.RESET_ALL}").lower() not in ['n', 'no']:
                try:
        ...
        try:
            ...
            while True:
                ...
                if choice == "1":
                    ...
                    if input(f"{Fore.RED}确认覆盖现有文件？[y/N]: {Style.RESET_ALL}").lower() in ['y', 'yes']:
    ...
    def _configure_core_settings(self):
        ...
        new_lang = input(f"{Fore.CYAN}默认语言（当前: {current}）[Enter保持不变]: {Style.RESET_ALL}").strip()
        if new_lang:
            self.config.core.default_language = new_lang
        
        # 源语言
        current = self.config.core.source_language
        new_lang = input(f"{Fore.CYAN}源语言（当前: {current}）[Enter保持不变]: {Style.RESET_ALL}").strip()
        if new_lang:
            self.config.core.source_language = new_lang
        
        # 调试模式
        current = "是" if self.config.core.debug_mode else "否"
        debug = input(f"{Fore.CYAN}调试模式（当前: {current}）[y/n/Enter保持]: {Style.RESET_ALL}").lower()
        if debug in ['y', 'yes']:
    ...
    def _configure_general_settings(self):
        ...
        auto_mode = input(f"{Fore.CYAN}启用自动模式（当前: {current_auto}）[y/n/Enter保持]: {Style.RESET_ALL}").lower()
    ...
    def _configure_api_settings(self):
        ...
        if input(f"{Fore.CYAN}是否修改阿里云Access Key ID？[y/N]: {Style.RESET_ALL}").lower() in ['y', 'yes']:
        ...
        if input(f"{Fore.CYAN}是否修改阿里云Access Key Secret？[y/N]: {Style.RESET_ALL}").lower() in ['y', 'yes']:
