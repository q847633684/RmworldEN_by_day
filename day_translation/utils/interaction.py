...
def show_main_menu() -> str:
    ...
    print(f"  1. 📤 提取模板  ── 提取翻译模板并生成 CSV 文件")
    print(f"  2. 🤖 Python机翻 ── 使用Python阿里云翻译 CSV 文件")
    print(f"  3. 📥 导入模板  ── 将翻译后的 CSV 导入翻译模板")
    
    print(f"\n{Fore.YELLOW}🛠️ 高级功能：{Style.RESET_ALL}")
    print(f"  4. 📚 语料生成  ── 生成英-中平行语料")
    print(f"  5. 🔄 完整流程  ── 提取、翻译、导入一键完成")
    print(f"  6. 📦 批量处理  ── 处理多个模组")
    print(f"  7. ⚙️ 配置管理  ── 管理翻译配置")
    print(f"  8. 🚀 Java机翻  ── 使用Java工具进行高性能翻译")
...
def select_csv_path_with_history() -> Optional[str]:
    ...
    while True:
        choice = input(f"{Fore.CYAN}请选择 (0-{len(csv_history)}, q退出): {Style.RESET_ALL}").strip()
        ...
        elif choice.isdigit() and 1 <= int(choice) <= len(csv_history):
