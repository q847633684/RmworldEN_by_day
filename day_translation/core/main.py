...
class TranslationFacade:
    ...
    def extract_templates_and_generate_csv(
        ...
        try:
            ...
            logging.info(
                f"开始提取模板: output_dir={output_dir}, en_keyed_dir={en_keyed_dir}, auto_choose_definjected={auto_choose_definjected}"
            )
            ...
            translations = self.template_manager.extract_and_generate_templates(
                output_dir, en_keyed_dir, auto_choose_definjected
            )
    ...
    def generate_corpus(self) -> List[Tuple[str, str]]:
        ...
        try:
            ...
            corpus = generate_parallel_corpus(
                self.mod_dir, CONFIG.source_language, self.language
            )
    ...
    def machine_translate(self, csv_path: str, output_csv: str = None) -> None:
        ...
        try:
            ...
            access_key_secret = self._get_api_key(
                "ALIYUN_ACCESS_KEY_SECRET", user_config
            )
    ...
    def _get_api_key(self, key_name: str, user_config: Dict[str, Any]) -> str:
        ...
        if not key:
            ...
            if (
                input(f"{Fore.YELLOW}保存到配置文件？[y/n]:{Style.RESET_ALL} ").lower()
                == "y"
            ):
...
def get_user_input_with_history(
    ...
    while True:
        if history:
            ...
            choice = input(
                f"\n{Fore.CYAN}选择历史记录 (0-{len(history)}) 或直接输入新路径：{Style.RESET_ALL}"
            ).strip()
...
def show_welcome():
    ...
    user_config = get_user_config()
    has_api_key = bool(
        user_config.get("ALIYUN_ACCESS_KEY_ID") or os.getenv("ALIYUN_ACCESS_KEY_ID")
    )
...
def main():
    ...
    try:
        ...
        while True:
            ...
            print(
                f"1. {Fore.GREEN}提取模板{Style.RESET_ALL}：提取翻译模板并生成 CSV 文件"
            )
            print(f"2. {Fore.GREEN}机翻{Style.RESET_ALL}：使用阿里云翻译 CSV 文件")
            print(
                f"3. {Fore.GREEN}导入模板{Style.RESET_ALL}：将翻译后的 CSV 导入翻译模板"
            )
            ...
            mode = (
                input(f"\n{Fore.CYAN}选择模式 (1-7, q):{Style.RESET_ALL} ")
                .strip()
                .lower()
            )
            ...
            try:
                if mode == "7":
                    ...
                    while True:
                        ...
                        config_mode = (
                            input(f"\n{Fore.CYAN}选择操作 (1-6, b):{Style.RESET_ALL} ")
                            .strip()
                            .lower()
                        )
                        ...
                        if config_mode == "1":
                            ...
                            item = (
                                input(
                                    f"\n{Fore.CYAN}选择要修改的项 (1-5, b):{Style.RESET_ALL} "
                                )
                                .strip()
                                .lower()
                            )
                            ...
                            try:
                                if item == "1":
                                    value = path_manager.get_path(
                                        path_type="default_language",
                                        prompt="请输入默认语言（例如：ChineseSimplified）: ",
                                        validator_type="str",
                                        default=path_manager.get_remembered_path(
                                            "default_language"
                                        ),
                                    )
                                    if value:
                                    ...
                                    value = path_manager.get_path(
                                        path_type="source_language",
                                        prompt="请输入源语言（例如：English）: ",
                                        validator_type="str",
                                        default=path_manager.get_remembered_path(
                                            "source_language"
                                        ),
                                    ...
                                    value = path_manager.get_path(
                                        ...
                                        prompt="请输入输出CSV路径（例如：output.csv）: ",
                                        validator_type="csv",
                                        default=path_manager.get_remembered_path(
                                            "output_csv"
                                        ),
                                    ...
                                    value = path_manager.get_path(
                                        path_type="log_file",
                                        prompt="请输入日志文件路径（例如：translation.log）: ",
                                        validator_type="file",
                                        default=path_manager.get_remembered_path(
                                            "log_file"
                                        ),
                            ...
                            if (
                                input(
                                    f"{Fore.YELLOW}确认重置配置？[y/n]:{Style.RESET_ALL} "
                                ).lower()
                                == "y"
                            ):
                                # 提示用户手动删除配置文件来重置
                                ...
                                if config_path:
                                    print(
                                        f"{Fore.YELLOW}请手动删除配置文件来重置配置：{Style.RESET_ALL}"
                                    )
                                    print(
                                        f"{Fore.CYAN}配置文件位置：~/.day_translation/config.json{Style.RESET_ALL}"
                                    )
                                else:
                                    print(
                                        f"{Fore.GREEN}✅ 当前已是默认配置{Style.RESET_ALL}"
                                    )
                        elif config_mode == "4":
                            config_path = path_manager.get_path(
                                path_type="config_export",
                                prompt="请输入导出配置路径（例如：config_export.json）: ",
                                validator_type="json",
                                default=path_manager.get_remembered_path(
                                    "config_export"
                                ),
                            ...
                            if config_path:
                                try:
                                    ...
                                    print(
                                        f"{Fore.GREEN}✅ 配置已导出到：{config_path}{Style.RESET_ALL}"
                                    )
                                except ConfigError as e:
                                    print(f"{Fore.RED}❌ {str(e)}{Style.RESET_ALL}")
                        elif config_mode == "5":
                            config_path = path_manager.get_path(
                                ...
                                prompt="请输入导入配置路径（例如：config_import.json）: ",
                                validator_type="json",
                                default=path_manager.get_remembered_path(
                                    "config_import"
                                ),
                            ...
                            rules_path = path_manager.get_path(
                                path_type="rules_config",
                                prompt="请输入自定义规则配置文件路径（例如：rules.json）: ",
                                validator_type="json",
                                default=path_manager.get_remembered_path(
                                    "rules_config"
                                ),
                            ...
                            if rules_path:
                                try:
                                    ...
                                    print(
                                        f"{Fore.GREEN}✅ 规则配置已加载{Style.RESET_ALL}"
                                    )
                                    CONFIG.save_user_config()
                                except ConfigError as e:
                                    print(f"{Fore.RED}❌ {str(e)}{Style.RESET_ALL}")
                ...
                if mode == "1":
                    ...
                    print(
                        f"1. {Fore.GREEN}模组内部{Style.RESET_ALL}（直接集成到模组Languages目录，适合开发模组）"
                    )
                    print(
                        f"2. {Fore.GREEN}外部目录{Style.RESET_ALL}（独立管理，适合翻译工作和分发）"
                    )

                    output_choice = input(
                        f"{Fore.CYAN}请输入选项编号（1/2，回车默认2）：{Style.RESET_ALL}"
                    ).strip()
                    ...
                    if output_choice == "1":
                        # 选择内部模式：直接输出到模组内部
                        print(
                            f"{Fore.GREEN}✅ 将生成模板到模组内部Languages目录{Style.RESET_ALL}"
                        )
                    ...
                    auto_en_keyed_dir = os.path.join(
                        mod_dir, "Languages", "English", CONFIG.keyed_dir
                    )
                    ...
                    if os.path.exists(auto_en_keyed_dir):  # 如果检测到英文Keyed目录存在
                        ...
                        print(
                            f"{Fore.GREEN}✅ 已自动检测并使用英文 Keyed 目录: {auto_en_keyed_dir}{Style.RESET_ALL}"
                        )
                    else:  # 未检测到英文Keyed目录
                        ...
                        if (
                            input(
                                f"{Fore.YELLOW}未检测到英文 Keyed 目录，是否手动指定？[y/n]:{Style.RESET_ALL} "
                            ).lower()
                            == "y"
                        ):
                            # 让用户手动选择英文Keyed目录路径
                    ...
                    facade.extract_templates_and_generate_csv(
                        output_dir, en_keyed_dir, auto_choose_definjected=False
                    )
                    ...
                    if (
                        input(
                            f"{Fore.YELLOW}指定输出文件？[y/n]:{Style.RESET_ALL} "
                        ).lower()
                        == "y"
                    ):
                    ...
                    if (
                        input(
                            f"{Fore.YELLOW}确认导入翻译到模板？[y/n]:{Style.RESET_ALL} "
                        ).lower()
                        == "y"
                    ):
                    ...
                    if (
                        input(
                            f"{Fore.YELLOW}是否指定英文 Keyed 目录？[y/n]:{Style.RESET_ALL} "
                        ).lower()
                        == "y"
                    ):
                        en_keyed_dir = path_manager.get_path(
                    ...
                    translations = facade.extract_templates_and_generate_csv(
                        export_csv, en_keyed_dir, auto_choose_definjected=True
                    )

                    if (
                        translations
                        and input(
                            f"{Fore.YELLOW}确认翻译并导入？[y/n]:{Style.RESET_ALL} "
                        ).lower()
                        == "y"
                    ):
                        output_csv = None
                        ...
                        final_csv = output_csv or export_csv.replace(
                            ".csv", "_translated.csv"
                        )
                    ...
                    if (
                        input(
                            f"{Fore.YELLOW}是否指定翻译 CSV？[y/n]:{Style.RESET_ALL} "
                        ).lower()
                        == "y"
                    ):
                        csv_path = path_manager.get_path(
                    ...
                    if results:
                        ...
                        for mod_dir, result in results.items():
                            ...
                            print(
                                f"{status} {Path(mod_dir).name}: {result.error or '成功'}"
                            )
