...
class TemplateManager:
    ...
    def __init__(
        ...
        self.generator = TemplateGenerator(
            str(self.mod_dir), language, template_location
        )
    ...
    def extract_and_generate_templates(
        ...
        if output_dir:
            ...
            self._generate_templates_to_output_dir(
                translations, output_dir, en_keyed_dir
            )
        else:
    ...
    def _extract_all_translations(
        self, definjected_mode: str = "defs"
    ) -> List[Tuple[str, str, str, str]]:
        """
        提取所有翻译数据
        ...
        if definjected_mode == "definjected":
            ...
            src_lang_path = get_language_folder_path(
                CONFIG.source_language, str(self.mod_dir)
            )
    ...
    def _generate_all_templates(
        self, translations: List[Tuple[str, str, str, str]], en_keyed_dir: str = None
    ):
        """生成所有翻译模板"""
        ...
        if keyed_translations:
            ...
            logging.info(
                f"生成 {len(keyed_translations)} 条 Keyed 模板"
            )  # 生成DefInjected模板
        if def_translations:
    ...
    def _generate_templates_to_output_dir(
        ...
        try:
            ...
            if keyed_translations:
                ...
                print(
                    f"{Fore.GREEN}✅ Keyed模板已生成: {keyed_dir}{Style.RESET_ALL}"
                )  # 生成DefInjected模板
            if def_translations:
                self._handle_definjected_structure_choice(
                    def_translations=def_translations,
    ...
    def _save_translations_to_csv(
        self, translations: List[Tuple[str, str, str, str]], csv_path: str
    ):
        """保存翻译数据到CSV文件"""
    ...
    def _handle_definjected_structure_choice(
        ...
        if src_definjected_dir.exists():
            ...
            print(
                f"1. {Fore.GREEN}保持原英文DefInjected结构{Style.RESET_ALL}（推荐：与原模组翻译文件一致）"
            )
            print(
                f"2. {Fore.GREEN}按原Defs目录结构生成{Style.RESET_ALL}（按原始定义文件组织）"
            )
            print(
                f"3. {Fore.GREEN}按DefType自动分组{Style.RESET_ALL}（传统方式：ThingDefs、PawnKindDefs等）"
            )
        else:
            ...
            print(
                f"1. {Fore.GREEN}按原Defs目录结构生成{Style.RESET_ALL}（推荐：按原始定义文件组织）"
            )
            print(
                f"2. {Fore.GREEN}按DefType自动分组{Style.RESET_ALL}（传统方式：ThingDefs、PawnKindDefs等）"
            )

        structure_choice = input(
            f"{Fore.CYAN}请输入选项编号（回车默认1）：{Style.RESET_ALL}"
        ).strip()
        ...
        location_suffix = (
            "到模组内部"
            if is_internal_mode
            else f": {Path(export_dir) / 'Languages' / self.language / 'DefInjected'}"
        )
        ...
        if src_definjected_dir.exists():
            ...
            if structure_choice == "2":
                ...
                print(
                    f"{Fore.GREEN}✅ DefInjected模板已生成（按Defs结构）{location_suffix}{Style.RESET_ALL}"
                )
                ...
                print(
                    f"{Fore.GREEN}✅ DefInjected模板已生成（按DefType分组）{location_suffix}{Style.RESET_ALL}"
                )
            else:
                ...
                print(
                    f"{Fore.GREEN}✅ DefInjected模板已生成（保持原结构）{location_suffix}{Style.RESET_ALL}"
                )
        else:
