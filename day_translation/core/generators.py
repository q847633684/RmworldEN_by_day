...
from ..utils.utils import XMLProcessor, sanitize_xml, save_json
...
class TemplateGenerator:
    ...
    def generate_keyed_template(
        self, en_keyed_dir: str, export_dir: str = None
    ) -> None:
        ...
        for en_xml_file in en_path.rglob("*.xml"):
            ...
            if tree:
                zh_root = self._create_keyed_xml_from_source(
                    tree.getroot() if self.processor.use_lxml else tree
                )
    ...
    def generate_keyed_template_from_data(
        self, keyed_translations: List[TranslationData], export_dir: str = None
    ) -> None:
    ...
    def generate_definjected_template(
        self, defs_translations: List[TranslationData], export_dir: str = None
    ) -> None:
        logging.info(f"{Fore.GREEN}正在生成 DefInjected 翻译模板...{Style.RESET_ALL}")
        base_dir = self.get_template_base_dir(export_dir)
        zh_definjected_dir = (
            base_dir / "Languages" / self.language / CONFIG.def_injected_dir
        )
        ...
        for def_type, fields in def_groups.items():
            ...
            logging.info(
                f"生成 DefInjected 模板: {def_type}Defs.xml{Style.RESET_ALL}{Style.RESET_ALL}"
            )
