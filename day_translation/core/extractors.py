...
def scan_defs_sync(
    mod_dir: str, def_types: Set[str] = None, language: str = CONFIG.source_language
) -> List[Tuple[str, str, str, str]]:
        ...
        if tree:
            ...
            for def_node in def_nodes:
                ...
                field_translations = _extract_translatable_fields_recursive(
                    def_node, def_type, def_name, content_filter, "", None, def_type
                )
                ...
                for field_path, text, tag in field_translations:
                    ...
                    translations.append(
                        (full_path, text, tag, str(xml_file.relative_to(defs_dir)))
                    )
...
def extract_definjected_translations(
    ...
    print(
        f"{Fore.GREEN}正在从 DefInjected 目录提取翻译数据（模组目录：{mod_dir}, 语言：{language}）...{Style.RESET_ALL}"
    )
    processor = XMLProcessor()
    content_filter = ContentFilter(CONFIG)
    translations: List[Tuple[str, str, str, str]] = []
    ...
    for xml_file in xml_files:
        ...
        if tree:
            ...
            for key, text, tag in processor.extract_translations(
                tree, context="DefInjected", filter_func=content_filter.filter_content
            ):
            ...
            logging.debug(
                f"从 {xml_file.name} 提取到 {len(file_translations)} 条DefInjected翻译"
            )
            translations.extend(file_translations)
        else:
    ...
    print(
        f"{Fore.GREEN}从DefInjected目录提取到 {len(translations)} 条翻译{Style.RESET_ALL}"
    )
    return translations
