...
def handle_extract_translate(
    ...
    if os.path.exists(src_def_injected_path):
        ...
        print(
            f"1. {Fore.GREEN}以英文 DefInjected 为基础{Style.RESET_ALL}（推荐用于已有翻译结构的情况）"
        )
        print(
            f"2. {Fore.GREEN}直接从 Defs 目录重新提取可翻译字段{Style.RESET_ALL}（推荐用于结构有变动或需全量提取时）"
        )

        choice = input(
            f"{Fore.CYAN}请输入选项编号（1/2，回车默认1）：{Style.RESET_ALL}"
        ).strip()
        ...
        print(
            f"{Fore.YELLOW}未找到英文 DefInjected 目录，将从 Defs 提取可翻译字段{Style.RESET_ALL}"
        )
        return "defs"


def cleanup_backstories_dir(
    mod_dir: str, export_dir: str, language: str = CONFIG.default_language
) -> None:
    """清理背景故事目录"""
    export_dir = str(Path(export_dir).resolve())
    lang_path = get_language_folder_path(language, export_dir)
...
def process_def_file_wrapper(
    args: Tuple[Path, List[Tuple[str, str, str, str]]],
) -> Tuple[str, List[Tuple[str, List[Tuple[str, str, str]]]]]:
...
def export_definjected(
    ...
    logging.info(
        f"导出 DefInjected: mod_dir={mod_dir}, translations_count={len(selected_translations)}"
    )
    mod_dir = str(Path(mod_dir).resolve())
    export_dir = str(Path(export_dir).resolve())
    lang_path = get_language_folder_path(language, export_dir)
...
def export_definjected_with_original_structure(
    ...
    logging.info(
        f"按原结构导出 DefInjected: mod_dir={mod_dir}, translations_count={len(selected_translations)}"
    )
    mod_dir = str(Path(mod_dir).resolve())
    export_dir = str(Path(export_dir).resolve())
    lang_path = get_language_folder_path(language, export_dir)
    ...
    if not os.path.exists(src_def_injected_path):
        logging.warning(
            f"原英文DefInjected目录不存在: {src_def_injected_path}，回退到默认结构"
        )
        # 回退到原来的函数
    ...
    logging.info(
        f"文件分组完成: {len(file_groups)} 个文件, {len(unmatched_translations)} 个未匹配"
    )
    ...
    for rel_path, translations in file_groups.items():
        ...
        processor.save_xml(tree, output_file, pretty_print=True)
        logging.info(
            f"生成 DefInjected 文件: {output_file} ({len(translations)} 条翻译)"
        )
    ...
    if unmatched_translations:
        ...
        print(
            f"{Fore.YELLOW}⚠️ 发现 {len(unmatched_translations)} 条未匹配的翻译，将生成到 _Additional.xml{Style.RESET_ALL}"
        )
...
def export_definjected_with_defs_structure(
    ...
    logging.info(
        f"按Defs结构导出 DefInjected: mod_dir={mod_dir}, translations_count={len(selected_translations)}"
    )
    mod_dir = str(Path(mod_dir).resolve())
    export_dir = str(Path(export_dir).resolve())
    lang_path = get_language_folder_path(language, export_dir)
