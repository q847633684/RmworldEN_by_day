"""
RimWorld 模组翻译占位符保护系统

这是一个专为 RimWorld 模组翻译设计的占位符保护和管理系统，确保翻译过程中
游戏占位符不被破坏，同时支持成人内容词汇的智能翻译。

主要功能：
1. 占位符保护：智能识别和保护各种类型的游戏占位符
   - 游戏占位符：{0_labelShort}, {VALUE}%, {RAPIST_possessive}
   - RimWorld前缀：r_logentry->, sent->, name->, pawn->
   - 通用占位符：[xxx], (PH_1), (PH_2)
   - 函数调用：pawn, pawn->

2. 成人内容翻译：内置成人内容词典，支持中英文混合文本翻译
   - 自动识别英文成人词汇
   - 支持中英文混合文本的精确匹配
   - 可配置的成人内容翻译策略

3. CSV文件处理：完整的CSV文件占位符保护流程
...
工作流程：
1. 保护阶段：扫描文本，识别占位符，用占位符标记替换
2. 翻译阶段：将保护后的文本发送给翻译API
3. 恢复阶段：将翻译结果中的占位符标记恢复为原始占位符
4. 成人内容翻译：在恢复阶段同时翻译剩余的英文成人词汇
