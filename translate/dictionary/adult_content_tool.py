...
def main():
    """主函数"""
    parser = argparse.ArgumentParser(description="成人内容翻译工具")
    ...
    # 创建翻译器
    translator = AdultContentTranslator()

    # 处理参数
    if args.stats:
        stats = translator.get_dictionary_stats()
        ui.print_info("📊 词典统计信息:")
    ...
    if args.reload:
        if translator.reload_dictionary():
            ui.print_success("✅ 词典重新加载成功")
        else:
            ui.print_error("❌ 词典重新加载失败")
        return

    if args.add:
        english, chinese = args.add
        if translator.add_custom_translation(english, chinese):
            ui.print_success(f"✅ 添加自定义翻译: {english} -> {chinese}")
        else:
            ui.print_error("❌ 添加自定义翻译失败")
        return
    ...
    if not output_csv:
        ...
        output_csv = str(input_path.parent / f"{input_path.stem}_adult_translated.csv")

    ui.print_info(f"🔍 开始处理成人内容翻译...")
    ui.print_info(f"   输入文件: {input_csv}")
    ui.print_info(f"   输出文件: {output_csv}")

    success = translate_adult_content_in_csv(input_csv, output_csv)

    if success:
        ui.print_success(f"✅ 成人内容翻译完成: {output_csv}")
    else:
        ui.print_error("❌ 成人内容翻译失败")
